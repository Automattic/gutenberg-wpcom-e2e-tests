...
async function openBlockNavigator() {
	...
	return page.waitForSelector('.editor-block-navigation__container');
	/* END OVERRIDE */
}
...
describe('Navigating the block hierarchy', () => {
	...
	it('should navigate using the block hierarchy dropdown menu', async () => {
		...
		const columnsBlockMenuItem = (await page.$x(
			"//button[contains(@class,'editor-block-navigation__item') and contains(text(), 'Columns')]"
		))[0];
		await columnsBlockMenuItem.click();

		// Tweak the columns count.
		await page.focus(
			'.edit-post-settings-sidebar__panel-block .components-range-control__number[aria-label="Columns"]'
		);
