...
xdescribe('Align Hook Works As Expected', () => {
	...
	const getAlignmentToolbarLabels = async () => {
		const buttonLabels = await page.evaluate(() => {
			return Array.from(
				document.querySelectorAll('.editor-block-toolbar button[aria-label^="Align"]')
			).map(button => {
	...
	const createDoesNotApplyAlignmentByDefaultTest = blockName => {
		it('Does not apply any alignment by default', async () => {
			...
			const pressedButtons = await page.$$(
				'.editor-block-toolbar button[aria-label^="Align"][aria-pressed="true"]'
			);
	...
	describe('Block with align true', () => {
		...
		createShowsTheExpectedButtonsTest(BLOCK_NAME, ['Align left', 'Align center', 'Align right']);
	...
	describe('Block with default align', () => {
		...
		const PRESSED_BUTTON_SELECTOR =
			'.editor-block-toolbar button[aria-label="Align right"][aria-pressed="true"]';
		createShowsTheExpectedButtonsTest(BLOCK_NAME, ['Align left', 'Align center', 'Align right']);
